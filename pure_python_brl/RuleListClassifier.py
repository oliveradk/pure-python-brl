...
import numpy as np
import pandas as pd
from pure_python_brl.LethamBRL.BRL_code import *
from pure_python_brl.Discretization.MDLP import *
import numbers
...
class RuleListClassifier(BaseEstimator):
    ...
    def __init__(self, listlengthprior=3, listwidthprior=1, maxcardinality=2, minsupport=10, alpha = np.array([1.,1.]), n_chains=3, max_iter=50000, class1label="class 1", discretizer=None, verbose=True):
    ...
    def fit(self, X, y, feature_labels = [], undiscretized_features = []):
        ...
        #note: removed _setdata() call (because refactored externally)
    ...
    def predict_proba(self, X):
        ...
        """
        #note: removed discretization step (done externally)
        D = X
        N = len(D)
